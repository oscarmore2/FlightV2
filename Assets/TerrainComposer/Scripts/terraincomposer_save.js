...
var generate_speed_manual: int = 10;
...
var object_speed_manual: int = 10;
...
class height_output_class
{
	var value: float;
	var strength: float = 1;
	var perlin_foldout: boolean = true;
}
...
function generate_begin(): int
{
	...
	for (count_terrain = 0;count_terrain < terrains.Count;++count_terrain)
    {
		...
		if (button_export)
		{
			if (!export_texture){export_texture = new Texture2D(preterrain.prearea.resolution,preterrain.prearea.resolution,TextureFormat.RGB24, false);}
		}
    ...
    if (heightmap_output)
	{
		heights = new float[preterrain.terrain.terrainData.heightmapResolution,preterrain.terrain.terrainData.heightmapResolution];
		// script_base.terrains[count_terrain].heights = terrains[count_terrain].heights;
	}
	
	if (grass_output)
	{
		grass_detail = new detail_class[preterrain.terrain.terrainData.detailPrototypes.Length];
		...
		for (var count_detail: int = 0;count_detail < grass_detail.Length;++count_detail) {
			...
			grass_detail[count_detail].detail = new int[preterrain.detail_resolution,preterrain.detail_resolution];
		}
				...
				switch(current_layer.output)
				{
					...
					case layer_output_enum.grass:
						if (subfilter_value*current_layer.strength > 0)
						{
							...
							if (settings.resolution_density)
							{
								...
								{
									grass_detail[count_grass].detail[detailmap_y,detailmap_x] += (preterrain.grass[count_grass]*settings.grass_density)/((prelayer3.prearea.resolution*settings.resolution_density_conversion)*(prelayer3.prearea.resolution*settings.resolution_density_conversion));
								}
							}
							else
							{
					...
					case layer_output_enum.object:
						if (subfilter_value*current_layer.strength*filter_strength > 0)
						{
							...
							if (settings.resolution_density && current_object.equal_density)
							{
								if (random_range2 > ((subfilter_value*current_layer.strength*filter_strength)/((prelayer3.prearea.resolution*settings.resolution_density_conversion)*(prelayer3.prearea.resolution*settings.resolution_density_conversion)))){continue;}
							}
							else
							...
							if (place)
							{
								...
								if (!current_object.prefab) {
									...
									object1.transform.rotation = Quaternion.Euler(rotation);
								}
								#endif
	...
	switch (filter.output)
	{
		...
		case condition_output_enum.multiply:
			...
			if (current_layer.output == layer_output_enum.color)
			{
				for (count_value = 0;count_value < current_layer.splat_output.splat.Count;++count_value)
				{
					preterrain.splat[current_layer.splat_output.splat[count_value]] *= color0[count_value]*(current_layer.strength*filter.strength*subfilter_value);
				}
			}
			else if (current_layer.output == layer_output_enum.splat)
			{
		...
		case condition_output_enum.devide:
			...
			else if (current_layer.output == layer_output_enum.grass)
			{
				for (count_value = 0;count_value < preterrain.grass.Length;++count_value)
				{
					if (current_layer.grass_output.grass_calc[current_layer.grass_output.grass[count_value].prototypeindex]*(current_layer.strength*filter.strength*subfilter_value) != 0){preterrain.grass[count_value] = preterrain.grass[count_value]/current_layer.grass_output.grass_calc[count_value]*(current_layer.strength*filter.strength*subfilter_value);}
				}
			}
			break;
		...
		case condition_output_enum.average:
			...
			else if (current_layer.output == layer_output_enum.grass)
			{
				for (count_value = 0;count_value < preterrain.grass.Length;++count_value)
				{
					preterrain.grass[current_layer.grass_output.grass[count_value].prototypeindex] += (current_layer.grass_output.grass_calc[count_value]*(current_layer.strength*filter.strength*subfilter_value))/current_layer.prefilter.filter_index.Count;		
				}
			}
			break;
		...
		case condition_output_enum.difference:
			...
			else if (current_layer.output == layer_output_enum.grass)
			{
				for (count_value = 0;count_value < preterrain.grass.Length;++count_value)
				{
					preterrain.grass[current_layer.grass_output.grass[count_value].prototypeindex] = Mathf.Abs(preterrain.grass[count_value]-(current_layer.grass_output.grass_calc[count_value]*(current_layer.strength*filter.strength*subfilter_value)));		
				}
			}
			break;
...
function calc_raw_value(raw: raw_class,local_x: float,local_y: float)
{
	...
	if (raw.flip_x){flip_x = -1;width = raw_files[raw.file_index[raw.raw_number]].resolution.x;} 
	if (raw.flip_y){flip_y = -1;height = raw_files[raw.file_index[raw.raw_number]].resolution.y;} 
	...
	if (raw.raw_mode == image_mode_enum.MultiTerrain)
	{
		...
		tile_x = ((((raw_files[raw.file_index[raw.raw_number]].resolution.x)/preterrain.tiles.x))*preterrain.tile_x);
		tile_y = (((raw_files[raw.file_index[raw.raw_number]].resolution.y/preterrain.tiles.y))*preterrain.tile_z);
	...
	if (preterrain1.terrain)
	{
		...
		if (command_first) 
		{
			if (settings.triplanar)
			{
				...
				preterrain1.script_triplanar = terrain_object.GetComponent(TriPlanarTerrainScript_TC);
			}
...
function find_terrain(first: boolean): boolean
{
	for (var count_terrain: int = prelayer.count_terrain;count_terrain < terrains.Count;++count_terrain)
	{
 		if (terrains[count_terrain].active) {
 			if (!first) {
	 			if (heightmap_output) {
	 				if ((terrains[count_terrain].heightmap_resolution*terrains[count_terrain].heightmap_resolution) != heights.Length) {
	 					heights = new float[terrains[count_terrain].heightmap_resolution,terrains[count_terrain].heightmap_resolution];
	 				}
	 			}
	 			if (grass_output) {
	 				if (terrains[count_terrain].terrain.terrainData.detailPrototypes.Length > grass_detail.Length-1) {grass_detail = new detail_class[terrains[count_terrain].terrain.terrainData.detailPrototypes.Length];}
 					for (var count_detail: int = 0;count_detail < grass_detail.Length;++count_detail) {
...
function assign_rtp(active: boolean)
{
	for (var count_terrain: int = 0;count_terrain < terrains.Count;++count_terrain) {
		if (!terrains[count_terrain].terrain) {continue;}
		if (active) {
			...
			if (!terrains[count_terrain].rtp_script) {
				Application.OpenURL ("https://www.assetstore.unity3d.com/#/content/5664");
...
function set_triplanar_script(preterrain1: terrain_class,mode: int)
{
	...
	{
		var terrain_object: GameObject = preterrain1.terrain.gameObject;
		preterrain1.script_triplanar = terrain_object.AddComponent(TriPlanarTerrainScript_TC);
	}
	else if (mode == -1)
	{
...
function check_synchronous_terrain_textures(preterrain1: terrain_class)
{
	if (color_output){check_synchronous_terrain_color_textures(preterrain1);}
		else {check_synchronous_terrain_splat_textures(preterrain1);}
}
