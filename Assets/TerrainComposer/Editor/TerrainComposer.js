...
class TerrainComposer extends EditorWindow 
{
	...
	{       
		...
		if (script.button_export)
		{
			...
			if (script.color_output)
			{
				if (script.settings.colormap){generate_window_height +=  gui_height;}
			}
		...
		{
			if (script.meshcapture_tool_foldout){generate_window_height += (gui_height*9)+3;}
		}
	    ...
	    if (global_script.settings.download_display)
	    {
	    	...
	    	if (map_tc) {
				...
				if (GUILayout.Button("Info here",GUILayout.Width(78))) {
			...
			GUILayout.Space(15);
			GUI.color = Color.green;
			EditorGUILayout.LabelField("Island and Examples Tutorial Package");		
		...
		if (script.settings.color_scheme_display)
        {
        	...
        	if (GUILayout.Button("X",EditorStyles.miniButtonMid,GUILayout.Width(19))) {
        		script.settings.color_scheme_display = false;
        	}
        	GUILayout.Space(2);
        	EditorGUILayout.EndHorizontal();
       		...
       		if (script.settings.color_scheme_display_foldout)
       		{
	        	...
	        	script.settings.color.backgroundColor = EditorGUILayout.ColorField(script.settings.color.backgroundColor,GUILayout.Width(150));
	        	gui_changed_old = GUI.changed;
	        	GUI.changed = false;
	        	script.settings.color.backgroundActive = EditorGUILayout.Toggle(script.settings.color.backgroundActive,GUILayout.Width(25));
	        	if (GUI.changed) {
			...
			if (script.quick_tools)
	        {
	        	...
	        	if (script.quick_tools_foldout)
	        	{
					if (script.settings.tabs)
					{
			        	...
			        	if (script.settings.global_parameters){GUI.backgroundColor = Color.white;}
			       		EditorGUILayout.EndHorizontal();
				    }
	        			...
	        			if (script.button_export && script.heightmap_output)
	        			{
	        				...
	        				for (count_terrain = 0;count_terrain < script.terrains.Count;++count_terrain)
	        				{
	        					if (script.terrains[count_terrain].active && script.terrains[count_terrain].terrain)
	        					{
	        						active = true;
	        						path_set = true;
	        						save_raw(script.terrains[count_terrain],script.terrains[0].raw_save_file);
        ...
        if (!global_script.auto_speed)
	    {
			...
			gui_changed_old = GUI.changed;
			GUI.changed = false;
			script.generate_speed_manual = EditorGUILayout.IntField(script.generate_speed_manual,GUILayout.Width(50));
			if (GUI.changed)
			{
				if (script.generate_speed_manual < 1){script.generate_speed_manual = 1;}
				if (script2)
		        {
		        	if (!script.auto_speed){script2.generate_speed = script.generate_speed_manual;}
		        }
		    }
		    GUI.changed = gui_changed_old;
		}
	    ...
	    if (GUILayout.Button("Auto",GUILayout.Width(50)))
	    {
	    	...
	    	if (script2)
	        {
	        	...
	        	if (global_script.auto_speed) {
	        		script2.generate_speed = 10000;
	        	}
	        	else {
	        		script2.generate_speed = script.generate_speed_manual;
	        	}
	        }
	...
	function generate_startup()
	{
		...
		if (script.color_output){set_terrain_color_textures();}
		else if (script.splat_output){set_terrain_splat_textures();}
			...
			if (current_terrain.color_terrain[0] > color_terrain[0]){current_terrain.color_terrain[0] -= 0.003;} 
	        	...
	        	if (current_terrain.foldout)
	        	{
	        		if (script.settings.tabs)
	        		{
		        		...
		        		if (GUILayout.Button(GUIContent("Scripts",button_script),EditorStyles.miniButtonMid,GUILayout.Width(95),GUILayout.Height(19)))
		        		{
	        		...
	        		if (current_terrain.data_foldout || script.settings.tabs)
	        		{
	        			...
	        			if (current_terrain.size_foldout)
	        			{
			        		...
			        		if (current_terrain.terrain)
			        		{
				        		...
				        		if (GUILayout.Button(GUIContent("<Get>",tooltip_text),GUILayout.Width(75))) {
				        			if (key.shift) {
				        				script.get_terrain_settings(current_terrain,"(siz)(con)");script.check_synchronous_terrain_size(current_terrain);
				        			}
				        			else {
		        		...
		        		if (current_terrain.splat_foldout)
		        		{
					    	...
					    	if (!script.settings.triplanar) {
			        			...
			        			if (GUILayout.Button(GUIContent("<RTP>",tooltip_text),GUILayout.Width(90))) {
			        				...
			        				if (key.shift) {
				        				if (current_terrain.rtp_script) {
				        					script.assign_rtp(false);
				        				}
				        				else {
				        					script.assign_rtp(true);
				        				}
				        			}
				        			else {
		        			...
		        			if (!current_terrain.rtp_script) {
			        			if (script.settings.triplanar){GUI.backgroundColor = Color.green;} else {GUI.backgroundColor = Color.white;}
			        			if (GUILayout.Button(GUIContent("<TriPlanar>",tooltip_text),GUILayout.Width(85)))
			        			{
			        				if (key.shift) {
				        				script.settings.triplanar = !script.settings.triplanar;
				        				triplanar_shader(script.settings.triplanar);
				        				script.set_all_terrain_splat_textures(current_terrain,false,false);
				        			}
				        			else {
										this.ShowNotification(GUIContent("Shift click <TriPlanar> to activate/deactivate the TriPlanar terrain shader"));
				}					}		
			        			GUI.backgroundColor = Color.white;
			        		}
		        			...
		        			if (!script.color_output) {
			        			...
			        			if (current_terrain.rtp_script) {
			        				...
			        				EditorGUILayout.LabelField("RTP Colormap");
			        				if (select_window) {GUI.backgroundColor = Color.green;}
			        				if (GUILayout.Button("Select",GUILayout.Width(65))) {
			        					if (!select_window){create_select_window();} else {select_window.Close();}
			        				}
			        				GUI.backgroundColor = Color.white;
			        				EditorGUILayout.EndVertical();
			        				EditorGUILayout.EndHorizontal();
			        		...
			        		if (current_terrain.terrain) {
			        			...
			        			gui_changed_window = GUI.changed; GUI.changed = false;
			        			current_terrain.terrain.materialTemplate = EditorGUILayout.ObjectField(current_terrain.terrain.materialTemplate,Material,true);
			        			if (GUI.changed)
			        			{
		        			...
		        			if (!script.color_output)
		        			{
			        			for (count_splat = 0;count_splat < current_terrain.splatPrototypes.Count;++count_splat)
			        			{
			        				...
			        				if (GUI.changed)
			        				{
			        					current_terrain.splatPrototypes[count_splat].import_max_size_list = get_image_import_max_size(current_terrain.splatPrototypes[count_splat].texture,false);
			        				}
			        				GUI.changed = gui_changed_old;
			        				#if !UNITY_3_4 && !UNITY_3_5
			        				if (!script.settings.triplanar) {
									        ...
									        */
											...
											else
											{
					        					...
					        					if (count_splat > -1)
					        					{
						        					EditorGUILayout.BeginHorizontal();
									        		GUILayout.Space(94+space);
									        		EditorGUILayout.LabelField("Splat Tile Size",GUILayout.Width(125));
													...
													EditorGUILayout.BeginHorizontal();
									        		GUILayout.Space(94+space);
									        		EditorGUILayout.LabelField("Normal Tile Size",GUILayout.Width(125));
												...
												EditorGUILayout.BeginHorizontal();
								        		GUILayout.Space(94+space);
								        		EditorGUILayout.LabelField("Splat Light",GUILayout.Width(125));
												...
												EditorGUILayout.BeginHorizontal();
								        		GUILayout.Space(94+space);
								        		EditorGUILayout.LabelField("Normal Light",GUILayout.Width(125));
					    ...
					    for (count_splat = 0;count_splat < current_layer.splat_output.splat.Count;++count_splat)
	           			{
  				        	...
  				        	{				    		
								...
								if (current_layer.splat_output.splat[count_splat] < script.terrains[current_layer.splat_output.splat_terrain].splatPrototypes.Count)
								{
									...
									EditorGUILayout.LabelField(GUIContent(script.terrains[current_layer.splat_output.splat_terrain].splatPrototypes[current_layer.splat_output.splat[count_splat]].texture),GUILayout.Width(50),GUILayout.Height(50));
								}
								gui_changed_old = GUI.changed;
								gui_changed_window = GUI.changed; GUI.changed = false;
			        ...
			        for (count_grass = 0;count_grass < current_layer.grass_output.grass.Count;++count_grass)
		           	{
		           		...
		           		if (current_layer.grass_output.grass[count_grass].prototypeindex < script.terrains[current_layer.grass_output.grass_terrain].detailPrototypes.Count)
						{
							...
							if (script.terrains[current_layer.grass_output.grass_terrain].detailPrototypes[current_grass.prototypeindex].prototypeTexture)
							{
					        ...
					        if (current_object.settings_foldout)
					        {
					        	GUILayout.Space(5);
			    	     		EditorGUILayout.BeginHorizontal();
				           		...
				           		if (GUILayout.Button("Create",GUILayout.Width(64)))
				           		{
				           			create_object_parent(current_object);
				           		}
				           		EditorGUILayout.EndHorizontal();
			...
			{	
				...
				if (preimage.image[count_image])
				{
					if (count_image == 0)
					{
						...
						preimage.import_max_size_list = get_image_import_max_size(preimage.image[0],true);
					}
	...
	*/
	...
	function triplanar_shader(active: boolean)
	{
		...
		if (active)
		{
			...
			var triplanar_script: TriPlanarTerrainScript_TC;
			
			for (count_terrain1 = 0;count_terrain1 < script.terrains.Count;++count_terrain1)
			{
				if (script.terrains[count_terrain1].terrain)
				{
					terrain_object = script.terrains[count_terrain1].terrain.gameObject;
					triplanar_script = terrain_object.GetComponent(TriPlanarTerrainScript_TC);
					if (!triplanar_script){script.terrains[count_terrain1].script_triplanar = script.terrains[count_terrain1].terrain.gameObject.AddComponent(TriPlanarTerrainScript_TC);}
						else {script.terrains[count_terrain1].script_triplanar = triplanar_script;}
					script.terrains[count_terrain1].script_triplanar.terDat = script.terrains[count_terrain1].terrain.terrainData;
					script.triplanar_init(script.terrains[count_terrain1]);
				}
			}
		}
		else
		{
			...
			for (count_terrain1 = 0;count_terrain1 < script.terrains.Count;++count_terrain1)
			{
				...
				{
					terrain_object = script.terrains[count_terrain1].terrain.gameObject;
					DestroyImmediate(terrain_object.GetComponent(TriPlanarTerrainScript_TC));
				}
			}
		}
	}
	...
	function init_color_splat_textures()
	{
		if (!script.settings.color_splatPrototypes[0].texture)
		{
			...
			set_image_import_settings(script.settings.color_splatPrototypes[0].texture,1,1,-1,-1);
			set_image_import_settings(script.settings.color_splatPrototypes[1].texture,1,1,-1,-1);
			set_image_import_settings(script.settings.color_splatPrototypes[2].texture,1,1,-1,-1);
		}
	}
	...
	function set_terrain_color_textures()
	{
		for (var count_terrain1: int = 0;count_terrain1 < script.terrains.Count;++count_terrain1)
		{
	...
	function get_image_import_max_size(texture: Texture2D,set_max: boolean): int
	{
		...
		return ((Mathf.Pow(textureImporter.maxTextureSize,0.2))+1);
	}
	...
	function save_raw(preterrain1: terrain_class,raw_file: raw_file_class)
	{
	    ...
	    if (raw_file.mode == raw_mode_enum.Mac)
	    {
		   	for (count_x = 0;count_x < resolution;++count_x) 
		   	{
				...
				{
					height = heights[count_x,count_y]*65535;
		...
		File.WriteAllBytes(raw_file.file+"_"+preterrain1.index.ToString()+".raw",bytes);
						...
						for (var count_index: int = 0;count_index < current_filter1.raw.file_index.Count;++count_index)
						{
							if (current_filter1.raw.file_index[count_index] > -1)
							{
								if (!script2.raw_files[current_filter1.raw.file_index[count_index]].loaded)
								{
									...
									if (script2.raw_files[current_filter1.raw.file_index[count_index]].bytes.Length == script2.raw_files[current_filter1.raw.file_index[count_index]].resolution.x*script2.raw_files[current_filter1.raw.file_index[count_index]].resolution.y*2) {
			...
			for (var row: float = 0;row < 2;++row)
			{
				for (var count_y: int = 0;count_y < script.terrains[0].tiles.y;++count_y)
				{
					...
					for (var count_terrain2: float = 0;count_terrain2 < 2;++count_terrain2)
					{
						...
						terrain_new.tree_instances.Clear();
						...
						for (count_tree1 = 0;count_tree1 < trees.Length;++count_tree1)
						{
							...
							if (tree.position.x < (0.5+(row/2)) && tree.position.x > (row/2))
							{
								if (tree.position.z < (1-(count_terrain2/2)) && tree.position.z > (0.5-(count_terrain2/2)))
								{
									...
									terrain_new.tree_instances.Add(tree);
								}
							} 
						}
													
						terrain_new.terrain.terrainData.treeInstances = new TreeInstance[terrain_new.tree_instances.Count];
						terrain_new.terrain.terrainData.treeInstances = terrain_new.tree_instances.ToArray();
					}
				}
			}
		}
	...
	function import_terrain_wc(name: String) 
	{
		...
		filter1.raw.tile_offset_x = (map_window.create_area.heightmap_offset.x*-1)+5;
	...
	function create_info_window()
	{
		...
		info_window.backgroundColor = script.settings.color.backgroundColor;
		info_window.backgroundActive = script.settings.color.backgroundActive;
