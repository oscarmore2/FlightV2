...
public class Skyshop : EditorWindow {
	...
	[SerializeField] private mset.CubemapGUI	inSKY = null;
	[SerializeField] private mset.CubemapGUI	outSKY = null;
	[SerializeField] private mset.CubemapGUI	outDIM = null;
	[SerializeField] private mset.CubemapGUI	outSIM = null;
	...
	public void OnEnable() {
		...
		if( outSKY == null ) outSKY = ScriptableObject.CreateInstance<mset.CubemapGUI>();
		if( outDIM == null ) outDIM = ScriptableObject.CreateInstance<mset.CubemapGUI>();
		if( outSIM == null ) outSIM = ScriptableObject.CreateInstance<mset.CubemapGUI>();		
		if( inSKY == null )	 inSKY = ScriptableObject.CreateInstance<mset.CubemapGUI>();
			
		inSKY.setType(mset.CubemapGUI.Type.INPUT,false);
		outSKY.setType(mset.CubemapGUI.Type.SKY,false);
		outDIM.setType(mset.CubemapGUI.Type.DIM,false);
		outSIM.setType(mset.CubemapGUI.Type.SIM,false);
				...
				if( uiRefOptions ) {				
					...
					EditorGUILayout.BeginHorizontal();{
						...
						if( GUILayout.Button( new GUIContent("Edit Active",""), GUILayout.Width(80), GUILayout.Height(32) ) ) {
							mset.Sky sky = mset.Sky.activeSky;
							//sky = uiSelectedSky;
							if( sky ) {
